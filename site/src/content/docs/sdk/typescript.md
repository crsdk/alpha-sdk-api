...
You also need a running server on `localhost:8080` (or wherever you point the client). Install the [server package](/alpha-sdk-api/web-api/server/) and start it:
...
For embedded server lifecycle inside a Node, Electron, or Tauri app, see the [`ServerManager` class](/alpha-sdk-api/web-api/server/#servermanager-nodejs).
...
See the [live-view polling recipe](/alpha-sdk-api/sdk/recipes/live-view-polling/) for a render loop.
...
By design, the following live in [recipes](/alpha-sdk-api/sdk/overview/#recipes) rather than the generated SDK:

- **SSE events** (`/api/events`, `/api/cameras/{id}/events`) — use native `EventSource` in the browser, or `fetch` + `ReadableStream` in Node 18+.
- **Discovery + reconnect orchestration** — copy from the [discovery + reconnect recipe](/alpha-sdk-api/sdk/recipes/discovery-reconnect/).
...
| `import { EventStream } from "camera-remote-web-api/server"` | See [SSE events recipe](/alpha-sdk-api/sdk/recipes/sse-events/) |
| `import { LiveViewStream } from "camera-remote-web-api/server"` | See [live-view polling recipe](/alpha-sdk-api/sdk/recipes/live-view-polling/) |
| `import { CameraManager } from "camera-remote-web-api/server"` | See [discovery + reconnect recipe](/alpha-sdk-api/sdk/recipes/discovery-reconnect/) |
