...
Pair `ServerManager` with the [TypeScript Client SDK](/alpha-sdk-api/sdk/typescript/) — start the server, connect to a camera, change a setting, fire the shutter, list and download the resulting photo, then disconnect:
...
| Python / Swift / other languages | Spawn the binary as a subprocess, or run `camera-server start` separately | See the [server subprocess example](/alpha-sdk-api/sdk/recipes/server-subprocess/). |

## Health and control endpoints

:::note
For TypeScript projects, use [`ServerManager`](/alpha-sdk-api/sdk/typescript/) plus the published client and example patterns instead of calling these endpoints directly in application code.
:::


See the auto-generated [API reference](/alpha-sdk-api/web-api/overview/) for full request and response schemas.
