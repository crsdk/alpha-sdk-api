...
<Tabs>
<TabItem label="CineEI / Log Shooting (Select LUT)">
...
<TabItem label="Picture Profile with PPLUT (Non-Log)">
...
`POST /api/cameras/{cameraId}/lut/import` uploads a `.cube` LUT file to the camera. The import is asynchronous — the endpoint returns `202 Accepted` and the result is delivered through the [`lutImportResult`](/alpha-sdk-api/web-api/events/#lutimportresult) SSE event.
...
1. **Set Exposure Index**

   Set `exposure-index` to the desired EI value.

1. **Select monitoring LUT**

   Set [`base-look-value`](/alpha-sdk-api/web-api/properties/#base-look-value) to the desired LUT.

1. **Enable LUT embedding (optional)**
...
1. **Select the monitoring LUT**

   Set [`base-look-value`](/alpha-sdk-api/web-api/properties/#base-look-value) to the imported LUT slot.
...
1. **Set a Custom Picture Profile**

   Set [`picture-profile`](/alpha-sdk-api/web-api/properties/#picture-profile) to `0x41`-`0x44` (Custom 1-4).

1. **Select the PPLUT slot**

   Set [`pp-lut-base-look`](/alpha-sdk-api/web-api/properties/#pp-lut-base-look) to the desired slot.
...
In volume photography workflows, write [`image-id-string`](/alpha-sdk-api/web-api/properties/#image-id-string) for the current subject and lock the shutter when no valid subject ID is present or the shot limit is reached.
