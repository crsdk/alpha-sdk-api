...
The TypeScript SDK's [`ServerManager`]({{ site.baseurl }}/web-api/server) provides the same lifecycle control programmatically. Use the CLI for scripts and one-off tasks; use `ServerManager` when the server's lifecycle is tied to your application's.
