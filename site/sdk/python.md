...
You also need a running server on `localhost:8080`. The Python client does **not** ship a `ServerManager` class — start the server using the Node-based CLI from the [server package]({{ site.baseurl }}/web-api/server), or spawn the binary as a subprocess from Python ([recipe]({{ site.baseurl }}/sdk/recipes/server-subprocess)).
