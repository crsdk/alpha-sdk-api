...
{: .note }
> Two things build on live view. Focus overlays are drawn from [AF Frame Position]({{ site.baseurl }}/web-api/actions#af-frame-position) and [Remote Touch and Tracking]({{ site.baseurl }}/web-api/actions#remote-touch-and-tracking), which report frame coordinates normalized to whatever size you render at. And the OSD frame paired with [Button Press]({{ site.baseurl }}/web-api/actions#button-press) is enough to drive the camera's own menus remotely — press, then read the screen back.
...
See [the live-view polling example]({{ site.baseurl }}/sdk/recipes/live-view-polling) for full client implementations in TypeScript, Python, and Swift.
